...
if ($hassiteconfig) {
    ...
    if ($ADMIN->fulltree) {
        $settings->add(new admin_setting_heading(
            'tool_coursemigration/backup',
            new lang_string('settings:backup', 'tool_coursemigration'),
            ''
        ));

        $settings->add(new admin_setting_configtext(
            'tool_coursemigration/destinationwsurl',
        ...
        $settings->add(new admin_setting_configcheckbox(
            'tool_coursemigration/failbackupdelete',
        ...
        $settings->add(new admin_setting_heading(
            'tool_coursemigration/restore',
            new lang_string('settings:restore', 'tool_coursemigration'),
            ''
        ));

        $settings->add(new admin_settings_coursecat_select(
            'tool_coursemigration/defaultcategory',
            get_string('settings:defaultcategory', 'tool_coursemigration'),
            get_string('settings:defaultcategorydesc', 'tool_coursemigration'),
            1
        ));

        $settings->add(new admin_setting_configcheckbox(
            'tool_coursemigration/hiddencourse',
        ...
        $settings->add(new admin_setting_configcheckbox(
            'tool_coursemigration/successfuldelete',
        ...
        $settings->add(new admin_setting_configcheckbox(
            'tool_coursemigration/failrestoredelete',
        ...
        $settings->add(new admin_setting_configtext(
            'tool_coursemigration/restoremaxretires',
        ...
        // Get the selected storage type and add its specific settings dynamically.
        $storagetype = get_config('tool_coursemigration', 'storagetype');
        if (!empty($storagetype) && class_exists($storagetype)) {
            try {
                ...
                debugging('Failed to instantiate storage type: ' . $e->getMessage(), DEBUG_DEVELOPER);
            }
        }
    }
    ...
    $ADMIN->add('coursemigration', new admin_externalpage(
        'coursemigrationupload',
